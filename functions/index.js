/**
 * Firebase Function HTTP (v2) com API Express.
 * Esta versão espelha a API local para facilitar migração.
 */
...
function getConnection(input = {}) {
  ...
  if (!baseUrl || !username || !password) throw new Error("serverUrl, username e password sao obrigatorios.");
...
async function fetchXtream(conn, action, extraParams = {}) {
  ...
  if (fromCache && fromCache.expiresAt > now) return { data: fromCache.data, cache: "HIT" };

  const params = new URLSearchParams({ username: conn.username, password: conn.password });
  if (action) params.set("action", action);
  for (const [k, v] of Object.entries(extraParams)) if (v != null && v !== "") params.set(k, String(v));
...
function paginateList(items, page, limit) {
  ...
  return { rows: items.slice(start, end), total, page, limit, totalPages: Math.max(1, Math.ceil(total / limit)) };
}
...
function withPlayUrl(kind, conn, item) {
  ...
  if (kind === "live") return { ...item, play_url: `${conn.baseUrl}/live/${conn.username}/${conn.password}/${id}.m3u8` };
  if (kind === "vod") {
...
function parseSeriesEpisode(episode, conn) {
  ...
  const ext = episode?.container_extension || episode?.info?.container_extension || episode?.episode_info?.container_extension || "mp4";
  return { ...episode, play_url: `${conn.baseUrl}/series/${conn.username}/${conn.password}/${episodeId}.${ext}` };
}
...
app.get("/api/series/:seriesId", async (req, res) => {
  try {
    ...
    const seasons = Object.entries(episodesMap).map(([season, episodes]) => {
      ...
      return { season: Number.parseInt(season, 10) || season, episodes: items.map((ep) => parseSeriesEpisode(ep, conn)).filter(Boolean) };
    });
    return ok(res, { series_id: seriesId, info: data?.info || {}, seasons }, { cache: cacheStatus, totalSeasons: seasons.length });
  } catch (error) {
